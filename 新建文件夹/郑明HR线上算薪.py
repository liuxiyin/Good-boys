...
class exceldeal(object):
    ...
    def excel_rewrite(self):
        ...
        for index, j in df1.iterrows():
            ...
            if type(j['姓名']) == str and j['姓名'] != '姓名' and j['姓名'].isdigit()==False:
                pass
            elif str(j['姓名'])=='nan' and str(j['身份证号']) != 'nan':
                pass
            else:
    ...
    def lenglian(self):
        list1 = [['姓名', 2, 1], ['身份证号', 3, 1], ['基本工资', 6, 2], ['岗位工资', 7, 2], ['应出勤', 12, 1], ['出勤天数', 13, 1],
                 ['迟到', 14, 1], ['调休、年假、公休', 15, 1], ['事假', 16, 1], ['病假', 17, 1], ['缺勤', 18, 1], ['货损扣款', 21, 2],
                 ['事假/缺勤', 22, 2], ['病假', 23, 2], ['其他扣款', 25, 2], ['补贴', 26, 2], ['加班', 27, 2],
                 ['高温津贴（餐费）', 30, 2], ['公里数补贴', 32, 2], ['服务奖', 33, 2], ['通讯费', 34, 2], ['安全奖', 35, 2],
                 ['专项附加扣除', 46, 1], ['个人社保', 49, 1], ['公司社保', 50, 1], ['个人公积金', 51, 1], ['公司公积金', 52, 1],
                 ['前锦网络公司社保', 62, 1], ['社保服务费', 64, 1], ['个调税', 71, 1], ['税后扣款', 74, 1]]
        list2 = [['计薪统计月', 4], ['发薪日期', 5], ['绩效工资', 8], ['拟岗位工资', 9], ['加班支付标准', 10], ['固定加班费', 11],
                 ['当月转正后实际出勤天数', 19], ['当月调薪后实际出勤天数', 20], ['考勤扣款合计', 24], ['饭贴（计算）', 28], ['饭贴调整', 29], ['高温费', 31],
                 ['补贴调整', 36], ['转正补差（计薪当月）', 37], ['转正补差调整', 38], ['实际转正补差', 39], ['调薪补差（计薪当月）', 40], ['调薪补差调整', 41],
                 ['实际调薪补差', 42], ['其他收入合计', 43], ['补发工资', 44], ['应发工资', 45], ['社保基数', 47], ['公积金基数', 48],
                 ['地方附加医疗（公司）', 53], ['社保补缴项（个人）', 54], ['社保补缴项（公司）', 55], ['公积金补缴项（个人）', 56], ['公积金补缴项（公司）', 57],
                 ['个人缴纳（派遣）', 58], ['个人社保手动调整（当月）', 59], ['公司社保手动调整（当月）', 60], ['个人缴纳合计', 61], ['公司缴纳合计', 63],
                 ['本期收入手动调整（累计）', 65], ['专项扣除手动调整（累计）', 66], ['社保公积金手动调整（累计）', 67], ['个税手动调整（累计）', 68],
                 ['至本月累计减除手动调整（累计）', 69], ['个人所得税(自动计算)', 70], ['个人所得税（实际）', 72], ['个税基数', 73], ['实发工资', 75],
                 ['人事总成本', 76], ['本期收入（参考）', 77], ['社保公积金（参考）', 78], ['其他减除（参考）', 79], ['专项附加扣除（参考）', 80],
                 ['个税（参考）', 81]]
        list_compare = ['F', 'G', 'L', 'M', 'N', 'O', 'P', 'Q', 'R', 'U', 'V', 'W', 'Y', 'Z', 'AA', 'AD', 'AF', 'AG',
                        'AH', 'AI', 'AT', 'AW', 'AX', 'AY', 'AZ', 'BJ', 'BL', 'BS', 'BV']
        self.file_deal(list1, list2, list_compare)

    def shangmao(self):
        list1 = [['姓名', 2, 1], ['身份证号', 3, 1], ['基本工资', 7, 2], ['岗位工资', 8, 2], ['拟岗位绩效', 9, 1], ['拟岗位', 10, 1],
                 ['迟到', 16, 1], ['应出勤', 17, 1], ['出勤天数', 18, 1], ['调休、年假、公休', 19, 1], ['事假', 20, 1], ['病假', 21, 1],
                 ['缺勤', 22, 1], ['缺勤扣款', 26, 2], ['扣款', 27, 2], ['事假', 28, 2], ['病假', 29, 2], ['绩效分', 31, 1],
                 ['绩效', 32, 2], ['餐费', 37, 2], ['通讯费', 40, 2], ['补贴', 46, 2], ['加班', 50, 2], ['全勤奖', 58, 2],
                 ['专项附加扣除', 63, 1], ['个人社保', 77, 1], ['公司社保', 78, 1], ['个人公积金', 79, 1], ['公司公积金', 80, 1],
                 ['外包社保', 89, 1], ['外包服务费', 91, 1], ['个调税', 99, 1], ['税后扣款', 102, 1]]
        list2 = [['计薪统计月', 4], ['发薪日期', 5], ['落户标准', 6], ['加班支付标准', 11], ['固定加班费', 12], ['加班计算基数', 13],
                 ['调休小时数（显示）', 14], ['年假天数（显示）', 15], ['司龄', 23], ['当月转正后实际出勤天数', 24], ['当月调薪后实际出勤天数', 25],
                 ['考勤扣款合计', 30], ['饭贴标准（月）', 33], ['饭贴标准（天）', 34], ['饭贴（计算）', 35], ['饭贴调整', 36], ['固定通讯费', 38],
                 ['通讯费（维护）', 39], ['房帖', 41], ['交通费', 42], ['服务奖', 43], ['技术奖', 44], ['油贴（元/月）', 45], ['平时加班小时数', 47],
                 ['周末加班小时数', 48], ['法定加班小时数', 49], ['补贴调整', 51], ['转正补差（计薪当月）', 52], ['转正补差调整', 53], ['实际转正补差', 54],
                 ['调薪补差（计薪当月）', 55], ['调薪补差调整', 56], ['实际调薪补差', 57], ['其他收入合计', 59], ['补发工资', 60], ['应发工资', 61],
                 ['落户标准补差', 62], ['社保基数', 64], ['公积金基数', 65], ['养老金（个人）', 66], ['养老金（公司）', 67], ['医疗金（个人）', 68],
                 ['医疗金（公司）', 69], ['失业金（个人）', 70], ['失业金（公司）', 71], ['大病医疗（个人）', 72], ['大病医疗（公司）', 73],
                 ['地方附加医疗（公司）', 74], ['生育保险', 75], ['工伤保险', 76], ['社保补缴项（个人）', 81], ['社保补缴项（公司）', 82],
                 ['公积金补缴项（个人）', 83], ['公积金补缴项（公司）', 84], ['个人缴纳（派遣）', 85], ['个人社保手动调整（当月）', 86], ['公司社保手动调整（当月）', 87],
                 ['个人缴纳合计', 88], ['公司缴纳合计', 90], ['本期收入手动调整（累计）', 92], ['社保公积金手动调整（累计）', 93], ['专项扣除手动调整（累计）', 94],
                 ['个税手动调整（累计）', 95], ['当月其他扣除', 96], ['至本月累计减除手动调整（累计）', 97], ['个人所得税(自动计算)', 98], ['个人所得税（实际）', 100],
                 ['个税基数', 101], ['实发工资', 103], ['人事总成本', 104], ['本期收入（参考）', 105], ['社保公积金（参考）', 106], ['其他减除（参考）', 107],
                 ['专项附加扣除（参考）', 108], ['个税（参考）', 109]]
        list_compare = ['G', 'H', 'I', 'J', 'P', 'Q', 'R', 'S', 'T', 'U', 'V', 'Z', 'AA', 'AB', 'AC', 'AE', 'AF', 'AK',
                        'AN', 'AT', 'AX', 'BF', 'BK', 'BY', 'BZ', 'CA', 'CB', 'CK', 'CM', 'CU', 'CX']
        self.file_deal(list1, list2, list_compare)

    def chengpei(self):
        list1 = [['姓名', 2, 1], ['身份证号', 3, 1], ['基本工资', 7, 2], ['岗位工资', 8, 2], ['拟岗位绩效', 9, 1], ['拟岗位', 10, 1],
                 ['迟到', 16, 1], ['应出勤', 17, 1], ['出勤天数', 18, 1], ['调休、年假、公休', 19, 1], ['事假', 20, 1], ['病假', 21, 1],
                 ['缺勤', 22, 1], ['缺勤扣款', 26, 2], ['迟到扣款', 27, 2], ['事假', 28, 2], ['病假', 29, 2], ['绩效', 31, 1],
                 ['绩效', 32, 2], ['餐费', 37, 2], ['通讯费', 40, 2], ['房帖', 41, 2], ['补贴', 46, 2], ['加班', 50, 2],
                 ['全勤奖', 58, 2], ['专项附加扣除', 63, 1], ['个人社保', 77, 1], ['公司社保', 78, 1], ['个人公积金', 79, 1],
                 ['公司公积金', 80, 1], ['前锦网络公司社保', 89, 1], ['社保服务费', 91, 1], ['个调税', 99, 1], ['税后扣款', 102, 1]]
        list2 = [['计薪统计月', 4], ['发薪日期', 5], ['落户标准', 6], ['加班支付标准', 11], ['固定加班费', 12], ['加班计算基数', 13],
                 ['调休小时数（显示）', 14], ['年假天数（显示）', 15], ['司龄', 23], ['当月转正后实际出勤天数', 24], ['当月调薪后实际出勤天数', 25],
                 ['考勤扣款合计', 30], ['饭贴标准（月）', 33], ['饭贴标准（天）', 34], ['饭贴（计算）', 35], ['饭贴调整', 36], ['固定通讯费', 38],
                 ['通讯费（维护）', 39], ['交通费', 42], ['服务奖', 43], ['技术奖', 44], ['油贴（元/月）', 45], ['平时加班小时数', 47],
                 ['周末加班小时数', 48], ['法定加班小时数', 49], ['补贴调整', 51], ['转正补差（计薪当月）', 52], ['转正补差调整', 53], ['实际转正补差', 54],
                 ['调薪补差（计薪当月）', 55], ['调薪补差调整', 56], ['实际调薪补差', 57], ['其他收入合计', 59], ['补发工资', 60], ['应发工资', 61],
                 ['落户标准补差', 62], ['社保基数', 64], ['公积金基数', 65], ['养老金（个人）', 66], ['养老金（公司）', 67], ['医疗金（个人）', 68],
                 ['医疗金（公司）', 69], ['失业金（个人）', 70], ['失业金（公司）', 71], ['大病医疗（个人）', 72], ['大病医疗（公司）', 73],
                 ...
                 ['专项附加扣除（参考）', 108], ['个税（参考）', 109]]
        list_compare = ['G', 'H', 'I', 'J', 'P', 'Q', 'R', 'S', 'T', 'U', 'V', 'Z', 'AA', 'AB', 'AC', 'AE', 'AF', 'AK',
                        'AN', 'AO', 'AT', 'AX', 'BF', 'BK', 'BY', 'BZ', 'CA', 'CB', 'CK', 'CM', 'CU', 'CX']
        self.file_deal(list1, list2, list_compare)

    def wenkong(self):
        list1 = [['姓名', 2, 1], ['身份证号', 3, 1], ['基本工资', 7, 2], ['岗位工资', 8, 2], ['拟岗位绩效', 9, 1], ['拟岗位', 10, 1],
                 ['迟到', 16, 1], ['应出勤', 17, 1], ['出勤天数', 18, 1], ['调休、年假、公休', 19, 1], ['事假', 20, 1], ['病假', 21, 1],
                 ['缺勤', 22, 1], ['病假', 26, 2], ['其他扣款', 27, 2], ['缺勤扣款', 28, 2], ['事假', 29, 2], ['绩效分', 31, 1],
                 ['绩效', 32, 2], ['餐费', 37, 2], ['通讯费', 40, 2], ['服务奖', 43, 2], ['技术奖', 44, 2], ['补贴', 46, 2],
                 ['加班费', 50, 2], ['专项附加扣除', 63, 1], ['个人社保', 77, 1], ['公司社保', 78, 1], ['个人公积金', 79, 1],
                 ['公司公积金', 80, 1], ['外包公司社保', 89, 1], ['社保服务费', 91, 1], ['个调税', 99, 1], ['税后扣款', 102, 1]]
        list2 = [['计薪统计月', 4], ['发薪日期', 5], ['落户标准', 6], ['加班支付标准', 11], ['固定加班费', 12], ['加班计算基数', 13],
                 ['调休小时数（显示）', 14], ['年假天数（显示）', 15], ['司龄', 23], ['当月转正后实际出勤天数', 24], ['当月调薪后实际出勤天数', 25],
                 ['考勤扣款合计', 30], ['饭贴标准（月）', 33], ['饭贴标准（天）', 34], ['饭贴（计算）', 35], ['饭贴调整', 36], ['固定通讯费', 38],
                 ['通讯费（维护）', 39], ['房帖', 41], ['交通费', 42], ['油贴（元/月）', 45], ['平时加班小时数', 47], ['周末加班小时数', 48],
                 ['法定加班小时数', 49], ['补贴调整', 51], ['转正补差（计薪当月）', 52], ['转正补差调整', 53], ['实际转正补差', 54], ['调薪补差（计薪当月）', 55],
                 ['调薪补差调整', 56], ['实际调薪补差', 57], ['全勤奖', 58], ['其他收入合计', 59], ['补发工资', 60], ['应发工资', 61],
                 ['落户标准补差', 62], ['社保基数', 64], ['公积金基数', 65], ['养老金（个人）', 66], ['养老金（公司）', 67], ['医疗金（个人）', 68],
                 ['医疗金（公司）', 69], ['失业金（个人）', 70], ['失业金（公司）', 71], ['大病医疗（个人）', 72], ['大病医疗（公司）', 73],
                 ['地方附加医疗（公司）', 74], ['生育保险', 75], ['工伤保险', 76], ['社保补缴项（个人）', 81], ['社保补缴项（公司）', 82],
                 ['公积金补缴项（个人）', 83], ['公积金补缴项（公司）', 84], ['个人缴纳（派遣）', 85], ['个人社保手动调整（当月）', 86], ['公司社保手动调整（当月）', 87],
                 ...
                 ['专项附加扣除（参考）', 108], ['个税（参考）', 109]]
        list_compare = ['G', 'H', 'I', 'J', 'P', 'Q', 'R', 'S', 'T', 'U', 'V', 'Z', 'AA', 'AB', 'AC', 'AE', 'AF', 'AK',
                        'AN', 'AQ', 'AR', 'AT', 'AX', 'BK', 'BY', 'BZ', 'CA', 'CB', 'CK', 'CM', 'CU', 'CX']
        self.file_deal(list1, list2, list_compare)

    def zongbu(self):
        list1 = [['姓名', 2, 1], ['身份证号', 3, 1], ['基本工资', 7, 2], ['岗位工资', 8, 2], ['拟岗位绩效', 9, 1], ['拟岗位', 10, 1],
                 ['迟到', 16, 1], ['应出勤', 17, 1], ['出勤天数', 18, 1], ['调休、年假、公休', 19, 1], ['事假、产假', 20, 1], ['病假', 21, 1],
                 ['缺勤', 22, 1], ['缺勤扣款', 26, 2], ['迟到扣款', 27, 2], ['事假', 28, 2], ['病假', 29, 2], ['绩效', 31, 1],
                 ['绩效', 32, 2], ['餐费', 37, 2], ['通讯费', 40, 2], ['房帖', 41, 2], ['补贴', 46, 2], ['加班', 50, 2],
                 ['全勤奖', 58, 2], ['专项附加扣除', 63, 1], ['个人社保', 77, 1], ['公司社保', 78, 1], ['个人公积金', 79, 1],
                 ['公司公积金', 80, 1], ['前锦网络公司社保', 89, 1], ['社保服务费', 91, 1], ['个调税', 99, 1], ['税后扣款', 102, 1]]
    ...
    def main(self, str_name):
        if '统嘉' in str_name:
            ...
            self.lenglian()
        elif '商贸' in str_name or '电商' in str_name:
            self.shangmao()
        elif '城配' in str_name:
            self.chengpei()
        elif '温控' in str_name:
...
class ZM(object):
    def __init__(self, playwright):
        ...
        self.browser = self.browser_type.launch(headless=False, executable_path=r'D:\python练习\ms-playwright\chromium-907428\chrome-win\chrome.exe'
                                                )
    ...
    def name_get(self):
        ...
        if '身份证号' not in df1.columns:
            ...
            logger.info('【日期（如2021年10月，且日期的两个数字放在前面）与识别的字段】放在[A1],确保有一列为“身份证号”,并且“身份证号”与“姓名”一样放在[第2行],没有"身份证号"的一行不能录入')
            raise
        ...
        if name == 'False':
            ...
            raise
        logger.info('表格读取成功')
        return name, time_year, time_month
    ...
    def select_ruler(self, name, main_iframe):
        ...
        dict_ruler = {'申雅': '汽配-申雅', '沈阳库基层': '薪资规则-5_15',
                      ...
                      '商贸': '薪资规则-1', '城配': '薪资规则-1', '温控': '薪资规则-1', '冷链': '薪资规则-2',
                      '电商': '薪资规则-1', '总部': '薪资规则-1'}
    ...
    def select_employee(self, name, main_iframe):
        ...
        dict_ruler = {'申雅': '汽配-申雅', '沈阳库基层': '薪资规则5-15（汽配-沈阳）',
                      ...
                      '入场安亭墨玉路': '汽配-墨玉路、其他', '墨玉路-其他': '汽配-墨玉路、其他',
                      '商贸': '薪资规则-1', '城配': '薪资规则-1', '温控': '薪资规则-1', '冷链': '薪资规则-2',
                      '电商': '薪资规则-1', '总部': '薪资规则-1'}  # 读取字典，获取选择的规则
        for i in dict_ruler:
            if i in name:
                find_ruler = dict_ruler[i]
    ...
    def update_excel(self, main_iframe):
        logger.info('上传文件')
    ...
    def main(self):
        name, time_year, time_month = self.name_get()
        self.login()  # 登录
        ...
        if main_iframe.is_visible(f'//span[text()="{name}"]'):
            logger.info(f'项目"{name}"存在，进入校验流程')
        else:
            logger.info(f'项目"{name}"不存在,可以用该名称创建新增项目')
        ...
        self.updata_file(os.getcwd() + '\\data\\' + '导入模板.xlsx', nass_name='导入模板')  # 日志：下载导入模板
        self.update_excel(main_iframe)  # 在 导入与导出界面 上传 已修改 的文件,点击全部计算，保存。回归"薪酬计算界面"
        time.sleep(5)
        file = self.file_download(name, main_iframe)  # 从"薪酬计算界面"进入 维护界面 里下载，留在导入与导出界面
        print(file)
...
if __name__ == '__main__':
    data_file = r'C:\Users\EDY\Desktop\9b1cf452dd9d44c6a9a00690577968ba(1).xlsx'
    # deal = exceldeal(r'D:\python练习\data\薪酬数据导入_20220111 (2).xls')
    # deal.main(r'2021年8月冷链')

    setting = {
        "recipientAddrs": '915709379@qq.com'
        # "recipientAddrs":'qinyu@zhengming-sh.com;chenxuejiao@zhengming-sh.com'
    }
